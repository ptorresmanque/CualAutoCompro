# admin-dialog-keyboard-navigation

Keyboard navigation standards for `AdminEditDialogComponent`: autofocus on first field, natural Tab order, ESC handling routed through the unsaved-changes guard.

## Purpose

Make the admin dialog usable without a mouse. Editors moving between many fields (especially versions with 28 fields) lose time when they have to reach for the mouse to start typing or to dismiss the dialog.
