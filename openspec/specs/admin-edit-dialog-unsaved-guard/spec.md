# admin-edit-dialog-unsaved-guard

Guards `AdminEditDialogComponent` against accidental loss of unsaved changes when the user attempts to close the dialog via the X button, Cancel button, or ESC key.

## Purpose

Prevent data loss from accidental dismissal of the admin edit dialog. The dialog is rendered inside a parent component (not as a real `MatDialog`), so closing it requires explicit guards on every close path.
