# admin-edit-dialog-required-marker

Visual marker (red asterisk for required, "opcional" badge for optional) on every field rendered by `AdminEditDialogComponent`. Derives from `FieldMeta.optional` in `entity-schemas.ts`.

## Purpose

Make required vs optional fields visually unambiguous for editors, reducing errors caused by users skipping mandatory fields or filling in optional ones that get silently dropped.
