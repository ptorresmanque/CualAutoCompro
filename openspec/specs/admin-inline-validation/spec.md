...
Inline validation errors on every field of `AdminEditDialogComponent`, rendered via `<mat-error>` from Angular Material after the user touches a control.

## Purpose

Replace the "submit-and-read-banner" UX with per-field error feedback so editors know exactly which field to fix without scanning text. Build on Material's existing `<mat-error>` infrastructure instead of inventing a new system.
