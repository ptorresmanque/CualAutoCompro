...
Optional help text per field via `FieldMeta.help` in `entity-schemas.ts`, rendered between the label and the input as a discreet hint.

## Purpose

Reduce friction on fields whose semantics aren't obvious (e.g. SOAP vs voluntary insurance, what unit kWh refers to). Help text is opt-in to avoid visual clutter.
