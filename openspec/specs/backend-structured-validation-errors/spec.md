# backend-structured-validation-errors

The backend `validation()` helper accepts an optional `ZodIssue[]` and exposes them as `error.fields[]` in the JSON response, enabling per-field error mapping on the client.

## Purpose

Provide structured per-field validation errors instead of concatenating them into a single message string. The frontend can map each `fields[i]` to the right `FormControl` for inline display.
