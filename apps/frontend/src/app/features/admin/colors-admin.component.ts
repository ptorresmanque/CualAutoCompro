...
export class ColorsAdminComponent {
  ...
  readonly items = signal<ColorRow[]>([]);
  readonly search = signal('');
  readonly pagination = signal({ page: 1, pageSize: 25, total: 0, totalPages: 1 });
  readonly page = signal(1);
  readonly pageSize = signal(25);
  readonly dialogEntity = signal<ColorRow | null | undefined>(undefined);
  ...
  readonly displayed = computed<ColorRow[]>(() => {
    const q = this.search().trim().toLowerCase();
    const filtered = q
      ? this.items().filter((c) => c.name.toLowerCase().includes(q))
      : this.items();
    return sortItems(filtered, this.sortKey(), this.sortDir(), (c, k) => c[k as SortKey]);
  });
  ...
  private async load(): Promise<void> {
    ...
    try {
      ...
      const res = await this.api.get<PagedResponse<ColorRow[]>>('/admin/colors', params);
  ...
  async onSave(value: Record<string, unknown>): Promise<void> {
    const c = this.dialogEntity();
    try {
      if (c) {
        await this.api.patch(`/admin/colors/${c.id}`, value);
        this.feedback.success(`Color "${value['name']}" actualizado`);
      } else {
        await this.api.post(`/admin/colors`, value);
        this.feedback.success(`Color "${value['name']}" creado`);
      }
      this.dialogEntity.set(undefined);
      await this.load();
    } catch (err) {
  ...
  async confirmDelete(row: ColorRow): Promise<void> {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      disableClose: true,
      data: {
        title: 'Eliminar color',
        message: `¿Eliminar color "${row.name}"?`,
    ...
    if (!ok) return;
    try {
      await this.api.delete(`/admin/colors/${row.id}`);
      this.feedback.success(`Color "${row.name}" eliminado`);
      await this.load();
    } catch (err) {
      const msg = (err as Error).message;
      this.error.set(msg);
